...
import {
  CLIENT_SDK_BRAND_VWO,
  CLIENT_SDK_BRAND_WINGIFY,
  type ClientSdkBrand,
} from './sdk-brand';

const LINKING_ERROR =
  `The package 'wingify-fme-react-native-sdk' (or legacy 'vwo-fme-react-native-sdk') doesn't seem to be linked. Make sure: \n\n` +
  Platform.select({ ios: "- You have run 'pod install'\n", default: '' }) +
  '- You rebuilt the app after installing the package\n' +
  '- You are not using Expo Go\n';

const FmeReactNativeSdk =
  NativeModules.VwoFmeReactNativeSdk ??
  NativeModules.FmeReactNativeSdk ??
  new Proxy(
    {},
    {
      get() {
...
/**
 * Event emitter instance for the native FME module.
 * Handles communication of log and integration events from the native side to JavaScript.
 */
const myModuleEmitter = new NativeEventEmitter(FmeReactNativeSdk);
...
/** Native FME bridge instance, typed to the {@link VWOBridgeInterface}. */
const VWONative: VWOBridgeInterface = FmeReactNativeSdk;

function invokeNativeInitialize(
  options: Record<string, unknown>,
  clientSdkBrand: ClientSdkBrand
): Promise<string> {
  if (
    clientSdkBrand === CLIENT_SDK_BRAND_WINGIFY &&
    typeof VWONative.initializeWingify === 'function'
  ) {
    return VWONative.initializeWingify(options);
  }
  if (typeof VWONative.initializeVwo === 'function') {
    return VWONative.initializeVwo(options);
  }
  if (typeof VWONative.initialize === 'function') {
    return VWONative.initialize(options);
  }
  return Promise.reject(
    new Error('VWO FME native module: initialize method is not available.')
  );
}

function isNativeInitializeAvailable(): boolean {
  return (
    typeof VWONative.initializeVwo === 'function' ||
    typeof VWONative.initializeWingify === 'function' ||
    typeof VWONative.initialize === 'function'
  );
}
...
interface VWOBridgeInterface {
  initializeVwo?(options: Record<string, unknown>): Promise<string>;
  initializeWingify?(options: Record<string, unknown>): Promise<string>;
  /** @deprecated Legacy bridge; prefer initializeVwo / initializeWingify */
  initialize?(options: Record<string, unknown>): Promise<string>;

  /** Retrieves a previously initialized SDK instance by its account ID and SDK key. */
  getInstance(
    accountId: number,
    sdkKey: string
  ): Promise<{ success: boolean; accountId: number; sdkKey: string }>;
...
export async function init(
  ...
  clientSdkBrand: ClientSdkBrand = CLIENT_SDK_BRAND_VWO
): Promise<any> {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      vwoInstance.clientSdkBrand = clientSdkBrand;
      ...
      if (!VWONative || !isNativeInitializeAvailable()) {
      ...
      const result = await Promise.race([
        invokeNativeInitialize(updatedOptions, clientSdkBrand),
        timeoutPromise,
      ]);
...
export class VWO {
  ...
  /** Whether this instance was created via `vwo.init` or `wingify.init`. */
  clientSdkBrand?: ClientSdkBrand;
  ...
  sendSdkInitTime = async (initTimeMs: number): Promise<void> => {
    ...
    if (accountId === 0 || !sdkKey) {
      throw new Error(
        'sendSdkInitTime requires accountId and sdkKey. Use an instance from init() or VWO.getInstance().'
      );
    }
