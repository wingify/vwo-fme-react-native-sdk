...
class VwoFmeReactNativeSdkModule(reactContext: ReactApplicationContext) :
  ...
  fun getFlag(featureKey: String, context: ReadableMap, promise: Promise) {
    val vwoUserContext = VWOUserContext().apply {
      ...
      this.customVariables =
        context.getMap("customVariables")?.toHashMap()?.filterValues { it != null } as? HashMap<String, Any> ?: HashMap<String, Any>()
    }
  ...
  fun trackEvent(eventName: String, context: ReadableMap, eventProperties: ReadableMap?) {
    ...
    val properties = eventProperties?.toHashMap()?.filterValues { it != null } as Map<String, Any> ?: emptyMap<String, Any>()
  ...
  fun sendSdkInitTime(initTimeMs: Double) {
    ...
    println("FME-App : sendSdkInitEvent inited @ ${initTimeMs}")
  }
